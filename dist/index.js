...
server.registerTool("search_contracts", {
    description: "Search live US state & local government contracts. Supports filters: states, NAICS codes (2-6 digits), keyword, agency, min/max value (USD), due date range. Returns contracts with title, agency, state, due date, set-asides, and value.",
...
server.registerTool("get_contract", {
    description: "Get full details for a single government contract by ID (including agency, set-asides, NAICS codes, dates, and attachments).",
    inputSchema: z.object({
...
server.registerTool("search_naics", {
    description: "Look up NAICS codes by keyword or code prefix. NAICS codes classify a business's industry — used to match contracts. Pass a keyword like 'janitorial' or 'IT services', or a code like '5415'.",
    inputSchema: z.object({
...
server.registerTool("score_contract", {
    description: "Score a single contract's fit against a contractor profile using GCS's 5-factor AI matching (NAICS, budget, location, keywords/skills, timeline). Returns a 0-100 score.",
    inputSchema: z.object({
        contract_id: z.string().describe("UUID of the contract"),
        profile: z
            .object({
    ...
    if (!res.ok) {
        throw new Error(body?.error?.message || `Match API error (HTTP ${res.status})`);
    }
    return {
...
server.registerTool("win_likelihood", {
    description: "Estimate the likelihood (0-100) that a contractor profile wins a specific contract, based on the historical award archetype that wins in the contract's category. Returns a score, grade, reasons, and the matched archetype.",
    inputSchema: z.object({
        contract_id: z.string().describe("UUID of the contract"),
        profile: z
            .object({
    ...
    const body = await res.json().catch(() => ({}));
    if (!res.ok) {
        throw new Error(body?.error?.message || `Win-likelihood API error (HTTP ${res.status})`);
    }
    return {
        content: [{ type: "text", text: JSON.stringify(body, null, 2) }],
    };
...
server.registerTool("archetypes", {
    description: "List the winning-business archetypes derived from historical government contract awards (e.g. IT & Software Services, regional construction prime). Each includes NAICS, typical award range, agency types, states, and sample winners.",
    inputSchema: z.object({
